...
export const getOffices = async (address: FormDataEntryValue) => {
  const data: civicinfo_v2.Schema$RepresentativeInfoResponse = await fetch(
    `${ENDPOINT}?key=${KEY}&address=${address}`
